...
function reverseCharacters(flipString) {
    if(typeof(flipString) === 'string') {
        return flipString.split('').reverse().join('');
    } else(typeof(flipString) === 'number')
    return Number(String(flipString).split('').reverse().join(''));
}
...
//https://www.tutorialspoint.com/what-are-the-differences-between-unshift-and-push-methods-in-javascript#:~:text=Both%20the%20methods%20are%20used,the%20end%20of%20the%20array.
